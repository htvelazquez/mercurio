...
var local_grabber_url =  'http://local.grabber.com',
	qa_grabber_url =  'https://grabber.mercurio.com',
	prod_grabber_url =  'https://grabber.mercurio.com';

var constantsFile = './src/js/constants.js',
...
gulp.task('copy', ['clean'], function() {
	var stream = gulp.src(['!'+constantsFile,'src/**/*'])
...
gulp.task('setconfig', ['copy'], function () {
	...
	switch(env) {
    	case 'production':
		case 'prod':
			return gulp.src(constantsFile)
				.pipe(replace( local_grabber_url , prod_grabber_url))
				.pipe(replace( qa_grabber_url , prod_grabber_url))
				.pipe(gulp.dest(destFile));

    	case 'qa':
		case 'test':
			return gulp.src(constantsFile)
				.pipe(replace( local_grabber_url , qa_grabber_url))
				.pipe(replace( prod_grabber_url, qa_grabber_url))
		  	    .pipe(gulp.dest(destFile));

		case 'dev':
		case 'local':
			return gulp.src(constantsFile)
				.pipe(replace( prod_grabber_url, local_grabber_url))
				.pipe(replace( qa_grabber_url, local_grabber_url))
				.pipe(gulp.dest(destFile));
...
gulp.task('build', ['clean','setversion','copy','setconfig'], function() {
	...
	var manifest = JSON.parse(fs.readFileSync(manifestFile)),
		distFileName = 'Grabber Extension v' + manifest.version + '.zip';
