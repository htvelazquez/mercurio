...
var local_hunter_url =  'http://local.hunter.com:32783',
	qa_hunter_url =  'https://qa-hunter.mercurio.com',
	prod_hunter_url =  'https://hunter.mercurio.com';
	...
	switch(env) {
		...
		case 'prod':
			return gulp.src(constantsFile)
				.pipe(replace( local_hunter_url , prod_hunter_url))
				.pipe(replace( qa_hunter_url , prod_hunter_url))
				.pipe(gulp.dest(destFile));

    	case 'qa':
		case 'test':
			return gulp.src(constantsFile)
				.pipe(replace( local_hunter_url , qa_hunter_url))
				.pipe(replace( prod_hunter_url, qa_hunter_url))
		  	    .pipe(gulp.dest(destFile));

		case 'dev':
		case 'local':
			return gulp.src(constantsFile)
				.pipe(replace( prod_hunter_url, local_hunter_url))
				.pipe(replace( qa_hunter_url, local_hunter_url))
				.pipe(gulp.dest(destFile));
...
gulp.task('build', ['clean','setversion','copy','setconfig'], function() {
	...
	var manifest = JSON.parse(fs.readFileSync(manifestFile)),
		distFileName = 'Hunter Extension v' + manifest.version + '.zip';
